...
void energyImpl::clear_import_request_schedule() {
    ...
    const bool apply_bsp_caps =
        !(mod->config.charge_mode == "DC" && mod->config.ignore_evse_board_caps_in_dc);
            ...
            for (auto& e : energy_flow_request.schedule_import) {
                if (!e.limits_to_root.ac_max_current_A.has_value() ||
                    ...
                    if (charger_state == Charger::EvseState::ChargingPausedEV &&
                        mod->config.request_zero_power_in_idle) {
...
void energyImpl::handle_enforce_limits(types::energy::EnforcedLimits& value) {
    ...
    if (value.uuid != energy_flow_request.uuid) {
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << fmt::format("evse_energy enforce ignored self={} target={} valid_for={}s", energy_flow_request.uuid,
                           value.uuid, value.valid_for);
        return;
    }

    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
        << fmt::format("evse_energy enforce begin self={} valid_for={}s", energy_flow_request.uuid, value.valid_for);
    ...
    if (value.uuid == energy_flow_request.uuid) {
        ...
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << "evse_energy publish_enforced_limits begin self=" << energy_flow_request.uuid;
        mod->p_evse->publish_enforced_limits(value);
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << "evse_energy publish_enforced_limits end self=" << energy_flow_request.uuid;
        ...
        if (value.limits_root_side.total_power_W.has_value()) {
            LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                << fmt::format("evse_energy publish_max_watt begin self={} watt={}W", energy_flow_request.uuid,
            ...
            mod->mqtt.publish(fmt::format("everest_external/nodered/{}/state/max_watt", mod->config.connector_id),
                              value.limits_root_side.total_power_W.value().value);
            LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                << "evse_energy publish_max_watt end self=" << energy_flow_request.uuid;
        ...
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << fmt::format("evse_energy set_max_current begin self={} limit={}A requested_limit={}A valid_for={}s",
        ...
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << fmt::format("evse_energy set_max_current end self={} result={}", energy_flow_request.uuid,
                           set_max_current_result);

        if (limit > 1e-5 || limit < -1e-5) {
            LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                << "evse_energy resume_charging_power_available begin self=" << energy_flow_request.uuid;
            mod->charger->resume_charging_power_available();
            LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                << "evse_energy resume_charging_power_available end self=" << energy_flow_request.uuid;
        }

        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << fmt::format("evse_energy signalNrOfPhasesAvailable begin self={} phases={}", energy_flow_request.uuid,
                           mod->ac_nr_phases_active);
        mod->signalNrOfPhasesAvailable(mod->ac_nr_phases_active);
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << "evse_energy signalNrOfPhasesAvailable end self=" << energy_flow_request.uuid;
        ...
        if (mod->config.charge_mode == "DC") {
            ...
            if (value.limits_root_side.total_power_W.has_value() and
                ...
                LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                    << "evse_energy get_ev_info begin self=" << energy_flow_request.uuid;
                ...
                LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                    << fmt::format("evse_energy get_ev_info end self={} target_voltage={}V actual_voltage={}V",
                                   energy_flow_request.uuid, target_voltage, actual_voltage);

                bool values_changed = true;
                LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                    << "evse_energy get_powersupply_capabilities begin self=" << energy_flow_request.uuid;
                auto powersupply_capabilities = mod->get_powersupply_capabilities();
                LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                    << "evse_energy get_powersupply_capabilities end self=" << energy_flow_request.uuid;
                ...
                if (values_changed) {
                    ...
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy update_dc_maximum_limits begin self=" << energy_flow_request.uuid;
                    mod->r_hlc[0]->call_update_dc_maximum_limits(evse_max_limits);
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy update_dc_maximum_limits end self=" << energy_flow_request.uuid;
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy update_dc_minimum_limits begin self=" << energy_flow_request.uuid;
                    mod->r_hlc[0]->call_update_dc_minimum_limits(evse_min_limits);
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy update_dc_minimum_limits end self=" << energy_flow_request.uuid;
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy inform_new_evse_max_hlc_limits begin self=" << energy_flow_request.uuid;
                    mod->charger->inform_new_evse_max_hlc_limits(evse_max_limits);
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy inform_new_evse_max_hlc_limits end self=" << energy_flow_request.uuid;
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy inform_new_evse_min_hlc_limits begin self=" << energy_flow_request.uuid;
                    mod->charger->inform_new_evse_min_hlc_limits(evse_min_limits);
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy inform_new_evse_min_hlc_limits end self=" << energy_flow_request.uuid;

                    // This is just neccessary to switch between charging and discharging
                    if (target_voltage > 0) {
                        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                            << "evse_energy apply_new_target_voltage_current begin self=" << energy_flow_request.uuid;
                        mod->apply_new_target_voltage_current();
                        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                            << "evse_energy apply_new_target_voltage_current end self=" << energy_flow_request.uuid;
                    }
                    ...
                    LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
                        << "evse_energy dc_limits unchanged self=" << energy_flow_request.uuid;
                }
            }
        }
        LOCAL_DIAG(local_diagnostics, LocalDiagnostics::Level::Info, LocalDiagnostics::Category::Energy)
            << "evse_energy enforce end self=" << energy_flow_request.uuid;
    }
}
